...
def check_git_status(repo="ultralytics/yolov5", branch="master"):
    ...
    splits = re.split(pattern=r"\s", string=subprocess.check_output("git remote -v", shell=True).decode())
    matches = [repo in s for s in splits]
    if any(matches):
        remote = splits[matches.index(True) - 1]
    else:
        remote = "ultralytics"
        subprocess.check_output(f"git remote add {remote} {url}", shell=True)
    subprocess.check_output(f"git fetch {remote}", shell=True, timeout=5)  # git fetch
    local_branch = subprocess.check_output("git rev-parse --abbrev-ref HEAD", shell=True).decode().strip()  # checked out
    n = int(subprocess.check_output(f"git rev-list {local_branch}..{remote}/{branch} --count", shell=True))  # commits behind
    if n > 0:
